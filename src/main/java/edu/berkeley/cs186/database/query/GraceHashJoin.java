...
public class GraceHashJoin {
    ...
    private List<Record> buildAndProbe(HashPartition partition) {
        ...
        while (buildRecords.hasNext()) {
            Record record = buildRecords.next();
            DataBox value = record.getValues().get(buildColumnIndex);
        ...
        while (probeRecords.hasNext()) {
            ...
            DataBox value = probeRecord.getValues().get(probeColumnIndex);
            if (hashTable.containsKey(value)) {
                for (Record lRecord : hashTable.get(value)) {
                    Record joinedRecord = joinRecords(lRecord, probeRecord, probeFirst);
    ...
    private List<Record> run(Iterator<Record> leftRecords, Iterator<Record> rightRecords, int pass) {
        ...
        for (HashPartition partition: partitions) {
            ...
            if (partition.getNumLeftPages() <= this.numBuffers - 2 || partition.getNumRightPages() <= this.numBuffers - 2) {
                List<Record> records = buildAndProbe(partition);
